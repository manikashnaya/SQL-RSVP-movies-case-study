...
SELECT COUNT(*) 
FROM movie;
-- Total count of rows : 7997
SELECT COUNT(*) 
FROM genre;
-- Total count of rows : 14662
SELECT COUNT(*) 
FROM director_mapping;
-- Total count of rows : 3867
SELECT COUNT(*) 
FROM names;
-- Total count of rows : 25735
SELECT COUNT(*) 
FROM ratings;
-- Total count of rows : 7997
SELECT COUNT(*) 
FROM role_mapping;
-- Total count of rows: 15615
...
SELECT (COUNT(*) - COUNT(id)) as id_null,
        ...
        (COUNT(*) - COUNT(worlwide_gross_income)) as worldwide_gross_income_null,
...
select country, count(id) from movie where country in ('USA', 'India') and year = 2019 group by country;
...
WITH genre_rank AS
(
	SELECT Genre as Genre_movie, COUNT(movie_id) AS movie_count,
...
SELECT 
		MIN(avg_rating) AS min_avg_ratings,
...
INNER JOIN Ratings AS Movie_Rating
ON movie_rating.movie_id = Movies.id
LIMIT 10;
...
SELECT genr.genre, COUNT(genr.movie_id) AS movie_count
FROM Genre AS genr
INNER JOIN ratings AS rating
ON genr.movie_id = rating.movie_id
INNER JOIN movie AS movies
ON movies.id = genr.movie_id
WHERE movies.country='USA' AND rating.total_votes>=1001 AND MONTH(date_published)=3 AND year=2017
GROUP BY genr.genre
ORDER BY movie_count DESC;
...
FROM movie AS Movie_name
INNER JOIN ratings AS Movie_rating
ON Movie_name.id = Movie_rating.movie_id
...
-- Country is Italy for the Italian Movie
SELECT SUM(total_votes) AS Total_vote_German_movie
FROM movie AS Movie_name
	INNER JOIN ratings AS Rating_movie
    ON Movie_Name.id = Rating_movie.movie_id
WHERE country like 'Germany';

-- Country is Italy for the Italian Movie
SELECT SUM(total_votes) AS Total_vote_Italian_movie
FROM movie AS Movie_name
	INNER JOIN ratings AS Rating_movie
    ON Movie_name.id = Rating_movie.movie_id
WHERE country=  'Italy';
...
SELECT genre,
		COUNT(g.movie_id),
        RANK() OVER(ORDER BY COUNT(g.movie_id) DESC) AS genre_rankings
FROM genre AS g
...
) AS genre_rankings_table

WHERE genre_rankings <=3
)
...
SELECT 
		DISTINCT name AS actor_name, COUNT(rat.movie_id) AS movie_count
FROM ratings AS rat
		INNER JOIN role_mapping AS rmap
		ON rmap.movie_id = rat.movie_id
		INNER JOIN names AS name
		ON rmap.name_id = name.id
...
SELECT title as Thriller_movie_name,
...
SUM(AVG(duration)) over w AS running_total_duration,
ROUND(avg(avg(duration)) over w,2) AS moving_avg_duration
FROM genre AS genr
INNER JOIN movie AS mov
ON mov.id = genr.movie_id
GROUP BY genre window w AS (ORDER BY avg(duration) rows unbounded preceding) ;


-- Round is good to have and not a must have; Same thing applies to sorting-  still applied :)
...
SELECT name_id AS director_id,
       ...
       COUNT(DISTINCT mo.id) AS number_of_movies,
       ROUND(DATEDIFF(MAX(date_published),MIN(date_published))/COUNT(DISTINCT mo.id)) AS avg_inter_movie_days,
